...
plt.semilogy(atom_sys.X_range, ground_state_exact, 'g-.', label='ground state via diagonalization')
plt.xlabel("$x$ (a.u.)")
...
# anything will a node should do the job
atom_sys.set_wavefunction(
    atom_sys.X_range * ground_state
)
...
for _ in xrange(4000):
    ...
    excited1_state -= ground_state * np.vdot(ground_state, excited1_state) * atom_sys.dX
...
plt.semilogy(atom_sys.X_range, np.abs(excited1_state), 'r-', label='exited state')
plt.semilogy(atom_sys.X_range, np.abs(excited1_state_after), 'b--', label='excited state after propagation')
plt.semilogy(atom_sys.X_range, np.abs(excited1_state_exact), 'g-.', label='excited state via diagonalization')
plt.ylim([1e-4, 1e0])
plt.xlabel("$x$ (a.u.)")
plt.legend(loc='lower center')
plt.show()