...
if __name__ == '__main__':
    ...
    plt.title('1D Dynamical Billiard (Poincare map)')
    ...
    plt.subplot(111, axisbg='grey')
    plt.title('2D Dynamical Billiard (Poincare map)')

    def grad_V(x1, x2):
        tmp = 10*np.exp(-x1**2-0.2*x2**2)
        return 2*x1*tmp, 0.4*x2*tmp

    # Initiate classical system
    sys = VerletIntegrator(
        V=lambda x1, x2: 10*(1. - np.exp(-x1**2-0.2*x2**2)),
        grad_V=grad_V, # defined above

        K=lambda p1, p2: 0.5*(p1**2 + p2**2),
        grad_K=lambda p1, p2: (p1, p2),
    ...
    # Set initial conditions
    sys.set_ensemble(
        X=np.random.uniform(-1.5, 1.5, (2, 10)),
        P=np.random.uniform(-1.5, 1.5, (2, 10))
    )
